...
const readmeKoPath = path.join(root, "README.ko.md");
...
if (!existsSync(readmeKoPath)) error("missing README.ko.md");
  ...
  if (!manifest || typeof manifest !== "object" || Array.isArray(manifest)) {
    ...
    if (!Array.isArray(manifest.mods) || manifest.mods.length === 0) {
      ...
      for (const rel of manifest.mods) {
        ...
        // Source is .ts for Letta package convention but intentionally uses JS-compatible syntax.
        // Copy to .mjs so node --check can parse it without a TypeScript loader.
        const dir = mkdtempSync(path.join(tmpdir(), "cruise-code-"));
...
console.log("cruise-code package check passed.");
